document.addEventListener('DOMContentLoaded', () => {
    ...
    cards.forEach((card, index) => {
        ...
        // Ensure card has an ID for linking
        card.id = `section-${id}`;
        
        const link = document.createElement('a');
        link.href = `#section-${id}`;
        link.className = 'toc-item';
        link.innerText = `${index + 1}. ${title.substring(0, 30)}${title.length > 30 ? '...' : ''}`;
    ...
    let progress = JSON.parse(localStorage.getItem('cryptosharia_progress') || '[]');
    ...
    window.toggleRead = function(id) {
        ...
        localStorage.setItem('cryptosharia_progress', JSON.stringify(progress));
        updateProgressUI();
    };

    updateProgressUI(); // initial UI update

    // 3. ScrollSpy / IntersectionObserver for TOC highlighting
        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (activeLink) {
                    ...
                    // auto scroll toc slightly if needed
